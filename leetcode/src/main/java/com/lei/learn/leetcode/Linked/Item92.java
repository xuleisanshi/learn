...
public class Item92 {
  public static ListNode reverseBetween(ListNode head, int m, int n) {
    ...
    if (head == null)
      return null;
    ListNode<Integer> dummy = new ListNode(0); // create a dummy node to mark the head of this list
    dummy.next = head;
    ListNode<Integer> pre = dummy; // make a pointer pre as a marker for the node before reversing
    for (int i = 0; i < m - 1; i++)
      pre = pre.next;
  ...
  public static void main(String[] args) {
    ...
    head.append(new ListNode<Integer>(2));
